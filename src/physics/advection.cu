...
__device__ int flatten(int col, int row, int z) {
    if (col >= GRID_COUNT_X || row >= GRID_COUNT_Y || z >= GRID_COUNT_Z || col < 0 || row < 0 || z < 0)
        printf("flatten oob");
    return idxClip(col, GRID_COUNT_X) + idxClip(row, GRID_COUNT_Y) * GRID_COUNT_Z + idxClip(z, GRID_COUNT_Z) * GRID_COUNT_X * GRID_COUNT_Y;
}
__device__ int vflatten(int col, int row, int z) {
    if (col >= GRID_COUNT_X + 1 || row >= GRID_COUNT_Y + 1 || z >= GRID_COUNT_Z + 1 || col < 0 || row < 0 || z < 0)
        printf("vflatten oob");
    return idxClip(col, GRID_COUNT_X + 1) + idxClip(row, GRID_COUNT_Y + 1) * (GRID_COUNT_Z + 1) + idxClip(z, GRID_COUNT_Z + 1) * (GRID_COUNT_X + 1) * (GRID_COUNT_Y + 1);
}
...
__global__ void resetKernelCentered(float* d_temp, float* d_oldtemp, float* d_smokedensity, float* d_oldsmokedensity) {
    ...
    if (d_abs(k_z - GRID_COUNT_Z / 2) * d_abs(k_z - GRID_COUNT_Z / 2) +
        d_abs(k_y - GRID_COUNT_Y / 2) * d_abs(k_y - GRID_COUNT_Y / 2) +
        d_abs(k_x - GRID_COUNT_X / 2) * d_abs(k_x - GRID_COUNT_X / 2) < GRID_COUNT_X * GRID_COUNT_Y / (5 * 5 * 25)) {
...
__global__ void resetKernelVelocity(float3* d_vel, float3* d_oldvel) {
    ...
    const int k_y = blockIdx.y * blockDim.y + threadIdx.y;
    const int k_z = blockIdx.z * blockDim.z + threadIdx.z;
    if ((k_x >= GRID_COUNT_X + 1) || (k_y >= GRID_COUNT_Y + 1) || (k_z >= GRID_COUNT_Z + 1)) return;
...
__device__ float3 getAlpham(float3* d_vel, float3 pos, int k) {
    ...
    for (unsigned int i = 0; i < SEMILAGRANGIAN_ITERS; i++) {
        ...
        uint3 b = { static_cast<unsigned int>(estimated.x / BLOCK_SIZE),
                   static_cast<unsigned int>(estimated.y / BLOCK_SIZE),
                   static_cast<unsigned int>(estimated.z / BLOCK_SIZE) };
...
__device__ float3 fconfinement(float3* d_vorticity, int k_x, int k_y, int k_z) {
    const int k = flatten(k_x, k_y, k_z);
    if (k_x == GRID_COUNT_X - 1 || k_y == GRID_COUNT_Y - 1 || k_z == GRID_COUNT_Z - 1)
...
//}
//
__global__ void velocityKernel(float* d_temp, float3* d_vel, float3* d_oldvel, float3* d_alpha_m, float* d_smokedensity, 
    float3* d_vorticity, float3 externalForce) {
    const int k_x = threadIdx.x + blockDim.x * blockIdx.x;
    const int k_y = threadIdx.y + blockDim.y * blockIdx.y;
    const int k_z = threadIdx.z + blockDim.z * blockIdx.z;
    if ((k_x >= GRID_COUNT_X) || (k_y >= GRID_COUNT_Y) || (k_z >= GRID_COUNT_Z)) return;
    ...
    //Boundary conditions
    if (k_x == 0 || k_x == GRID_COUNT_X - 1)
        d_vel[k].x = 0;
    if (k_y == 0 || k_y == GRID_COUNT_Y - 1)
        d_vel[k].y = 0;
    if (k_z == 0 || k_z == GRID_COUNT_Z - 1)
        d_vel[k].z = 0;
...
__device__ float scalarLinearInt(float* scalarfield, float3 pos, float oobvalue) {
    ...
    //bound check
    if (x <= 0 || x >= GRID_COUNT_X || y <= 0 || y >= GRID_COUNT_Y || z <= 0 || z >= GRID_COUNT_Z)
        return oobvalue;
...
__device__ float laplacian(float* field, float oobvalue, int k_x, int k_y, int k_z) {
    ...
    out += k_x < GRID_COUNT_X - 1 ? field[flatten(k_x + 1, k_y, k_z)] : oobvalue;
    out += k_y < GRID_COUNT_Y - 1 ? field[flatten(k_x, k_y + 1, k_z)] : oobvalue;
    out += k_z < GRID_COUNT_Z - 1 ? field[flatten(k_x, k_y, k_z + 1)] : oobvalue;
...
__global__ void tempAdvectionKernel(float* d_temp, float* d_oldtemp, float3* d_vel, float3* d_alpha_m, float* lap) {
    ...
    const int k_z = threadIdx.z + blockDim.z * blockIdx.z;
    if ((k_x >= GRID_COUNT_X) || (k_y >= GRID_COUNT_Y) || (k_z >= GRID_COUNT_Z)) return;
    const int k = flatten(k_x, k_y, k_z, GRID_COUNT_X, GRID_COUNT_Y, GRID_COUNT_Z);
    // Advection
    float3 pos = make_float3((k_x + 0.5f) * BLOCK_SIZE, (k_y + 0.5f) * BLOCK_SIZE, (k_z + 0.5f) * BLOCK_SIZE);
    //float3 pos = make_float3((k_x)*BLOCK_SIZE, (k_y)*BLOCK_SIZE, (k_z)*BLOCK_SIZE);
    //float3 alpha_m = getAlpham(d_vel, pos, k);
...
// TODO add block params to launcher
void kernelLauncher(uchar4* d_out,
    ...
    float* d_lap;
    float3* d_alpha_m;
    HANDLE_ERROR(cudaMalloc(&d_lap, GRID_COUNT_X * GRID_COUNT_Y * GRID_COUNT_Z * sizeof(float)));
    HANDLE_ERROR(cudaMalloc(&d_alpha_m, GRID_COUNT_X * GRID_COUNT_Y * GRID_COUNT_Z * sizeof(float3)));
...
void resetVariables(float* d_temp,
    ...
    const dim3 gridSizeC(blocksNeeded(GRID_COUNT_X, M_in.x),
        blocksNeeded(GRID_COUNT_Y, M_in.y),
        blocksNeeded(GRID_COUNT_Z, M_in.z));
    const dim3 gridSizeV(blocksNeeded(GRID_COUNT_X + 1, M_in.x),
        blocksNeeded(GRID_COUNT_Y + 1, M_in.y),
        blocksNeeded(GRID_COUNT_Z + 1, M_in.z));
